 ...
 * login page.
 */
CASAuthentication.prototype.bounce = function(ctx, next) {

    // Handle the request with the bounce authorization type.
    this._handle(ctx, next, AUTH_TYPE.BOUNCE);
};
 ...
 * login page.
 */
CASAuthentication.prototype.bounce_redirect = function(ctx, next) {

    // Handle the request with the bounce authorization type.
    this._handle(ctx, next, AUTH_TYPE.BOUNCE_REDIRECT);
};
 ...
 */
CASAuthentication.prototype.block = function(ctx, next) {

    // Handle the request with the block authorization type.
    this._handle(ctx, next, AUTH_TYPE.BLOCK);
};

/**
 * Handle a request with CAS authentication.
 */
CASAuthentication.prototype._handle = function(ctx, next, authType) {
    ...
    // If there is a CAS ticket in the query string, validate it with the CAS server.
        ...
        this._handleTicket(ctx, next);
    }
    // Otherwise, redirect the user to the CAS login.
    else {
        this._login(ctx, next);
    }
};
 ...
 */
CASAuthentication.prototype._login = function(ctx, next) {
 ...
 */
CASAuthentication.prototype.logout = function(ctx, next) {
 ...
 */
CASAuthentication.prototype._handleTicket = function(ctx, next) {
        ...
        response.on('end', function() {
            this._validate(body, function(err, user, attributes) {
                if (err) {
                    ...
                    res.redirect(ctx.session.cas_return_to);
                }
            }.bind(this));
        }.bind(this));
